...
rt_uint8_t *rt_hw_stack_init(void       *tentry,
                             ...
                             void       *texit)
{
    struct stack_frame_fpu *stack_frame;
    rt_uint8_t             *stk;
    unsigned long           i;

    stk  = stack_addr + sizeof(rt_uint32_t);
    stk  = (rt_uint8_t *)RT_ALIGN_DOWN((rt_uint32_t)stk, 8);
    /* 使用含 FPU 空间的栈帧，与 PendSV handler 恢复逻辑匹配 */
    stk -= sizeof(struct stack_frame_fpu);

    stack_frame = (struct stack_frame_fpu *)stk;

    /* init all register */
    for (i = 0; i < sizeof(struct stack_frame_fpu) / sizeof(rt_uint32_t); i ++)
    {
        ((rt_uint32_t *)stack_frame)[i] = 0x00000000;  /* 清零，bit0=0 避免 FPU 误判 */
    }
...
#if USE_FPU
    stack_frame->flag = 0;  /* flag=0: FPU active 保存路径，与 stack_frame_fpu 布局一致 */
#endif
