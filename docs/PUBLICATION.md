...
- 把离线仿真、协议回放或数值收敛写成真实机械臂验收结果；如只有操作者口述，必须标为 `operator_reported`，并明确缺少原始日志和统计证据；
...
真实上机必须另建现场记录，包含设备身份、固件哈希、标定版本、急停检查、反馈新鲜度和操作者确认。若现场附件尚未归档，只能先记录定性 `operator_reported` 结果，不能补写数值指标。GitHub 提交本身不等于硬件验收。
