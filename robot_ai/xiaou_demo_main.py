...
def print_payload(payload: dict) -> None:
    print("robot_cmd:", json.dumps(payload, ensure_ascii=False))
    print("mcu_frame:", frame_to_hex(encode_motion_frame(payload)))
...
def main() -> None:
    ...
    try:
        if args.text:
            cap, _reply = handle_command(args.text, model, cap, args.send_serial, args.speak)
            return
        ...
        while True:
            try:
                ...
                if command_text:
                    cap, _reply = handle_command(command_text, model, cap, args.send_serial, args.speak)
                if args.once:
                    return
