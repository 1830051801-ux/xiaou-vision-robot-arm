...
from common import PROJECT_DIR, ask_cloud_intent, print_json, serial_enabled
...
def print_pick_summary(payload: dict) -> None:
    ...
    print("[5/6] MCU binary frame:", frame_to_hex(frame))
...
def handle_text(user_text: str, model: OpenCVDnnYolo, send_serial: bool, speak: bool) -> None:
    ...
    if send_serial or serial_enabled():
        print("[5/6] sending to STM32 serial...")
        send_robot_payload(payload)
    else:
        print("[5/6] serial disabled, only printed frame. Set ENABLE_SERIAL=true or add --send-serial to send.")
...
def main() -> None:
    ...
    parser.add_argument("--send-serial", action="store_true", help="really send frame to STM32")
    ...
    print("=" * 60)
    print("小U一体化测试：语音/文字 -> 意图 -> YOLO -> 坐标 -> 串口帧")
    print("=" * 60)
    print("Loading YOLO...")
    model = OpenCVDnnYolo()
    print("Ready.")
    ...
    if args.text:
        handle_text(args.text, model, args.send_serial, args.speak)
        return
    ...
    while True:
        ...
        try:
            ...
            handle_text(user_text, model, args.send_serial, args.speak)
