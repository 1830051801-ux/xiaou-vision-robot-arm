...
while true; do
  ...
  echo "19. UART self-check (auto serial0 / TX 0xFF / RX 0xFF)"
  echo "20. USB serial detect"
  echo "21. STM32 motion frame loop (every 5s)"
  echo "0. Exit"
  echo ""
  ...
  case "$choice" in
    ...
    7) python robot_ai/06_serial_send_test.py ;;
    ...
    19) bash scripts/run_uart_test.sh ;;
    20) python robot_ai/15_usb_serial_detect.py ;;
    21) bash scripts/run_stm_motion_loop.sh ;;
    0) exit 0 ;;
    *) echo "Invalid choice." ;;
  esac
  echo ""
