...
class TargetPoseNode(Node):
    def __init__(self) -> None:
        ...
        self.yolo_module = importlib.import_module("vision.unified_yolo")
        self.targeting_module = importlib.import_module("vision_targeting")
        model_value = str(self.get_parameter("yolo_model").value).strip() or None
        profile_value = str(self.get_parameter("yolo_profile").value).strip() or None
        if model_value and profile_value:
            raise RuntimeError("yolo_model and yolo_profile are mutually exclusive")
        imgsz_value = int(self.get_parameter("yolo_imgsz").value)
        conf_value = float(self.get_parameter("yolo_conf").value)
        detector_factory = (
            self.yolo_module.UnifiedYolo.from_profile
            if profile_value
            else self.yolo_module.UnifiedYolo.from_config
        )
        self.detector = detector_factory(
            profile_value if profile_value else model_value,
            imgsz=imgsz_value if imgsz_value > 0 else None,
            conf=conf_value if conf_value > 0.0 else None,
            device=str(self.get_parameter("yolo_device").value),
            fallback_uncovered_classes=not bool(self.get_parameter("yolo_union").value),
        )
        self.get_logger().info(f"YOLO backends: {self.detector.backend_names}")
    ...
    def on_image(self, message: Image) -> None:
        ...
        try:
            ...
            target_aliases = {
                self.yolo_module.canonical_name(item)
                for item in self.targeting_module.TARGET_ALIASES.get(target_class, {target_class})
            }
            candidates = [
                item for item in self.detector.detect(frame) if self.yolo_module.canonical_name(item.name) in target_aliases
            ]
