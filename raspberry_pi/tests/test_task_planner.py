...
def complete_profile() -> dict:
    ...
    return {
        "schema_version": 2,
        "height_reference": "relative_to_table_m",
        "camera_contract": {
            "width_px": 1920,
            "height_px": 1080,
...
class TaskPlannerTests(unittest.TestCase):
    ...
    def test_profile_without_table_relative_height_reference_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            profile = complete_profile()
            profile["height_reference"] = "absolute_base_m"
            profile_path, homography_path = self._write_inputs(root, profile=profile)
            plan = plan_task(
                "pick",
                [DetectionCenter("cup", 300.0, 200.0, 0.9)],
                image_width_px=1920,
        ...
        self.assertFalse(plan.transmittable)
        self.assertIn("relative_to_table_m", plan.reason)
