...
class ArmControlTests(unittest.TestCase):
    ...
    def test_default_hardware_configuration_keeps_motion_locked(self) -> None:
        ...
        self.assertIn("mcu_calibration_verified", readiness.missing_or_invalid)
    ...
    def test_complete_measured_hardware_configuration_can_pass_gate(self) -> None:
        ...
        config["mcu_calibration"] = {
            ...
            "record_version": 1,
            "record_crc32": "A1B2C3D4",
        }
        readiness = validate_motion_readiness(config)
        self.assertTrue(readiness.ready, readiness)

    def test_hardware_gate_requires_the_exact_f407_protocol_identity(self) -> None:
        config = copy.deepcopy(load_hardware_config())
        config.update(
            {
                "motion_enabled": True,
        ...
        config["protocol_family"] = "unverified_firmware"
        config["transport"]["protocol_version"] = "unverified_firmware"
        readiness = validate_motion_readiness(config)
        self.assertIn("protocol_family", readiness.missing_or_invalid)
        self.assertIn("transport.protocol_version", readiness.missing_or_invalid)

    def test_fresh_live_calibration_status_can_pass_without_invented_crc(self) -> None:
        config = copy.deepcopy(load_hardware_config())
        config.update(
            {
                "motion_enabled": True,
        ...
        config["mcu_calibration"] = {
            "authority": "stm32_f407",
            "verification_method": "live_get_calib_status",
            "verified_unix_s": time.time(),
            "zero_valid": [True] * 6,
        }
        readiness = validate_motion_readiness(config)
        self.assertTrue(readiness.ready, readiness)
        config["mcu_calibration"]["verified_unix_s"] = time.time() - 31.0
        readiness = validate_motion_readiness(config)
        self.assertIn("mcu_calibration.live_timestamp", readiness.missing_or_invalid)
