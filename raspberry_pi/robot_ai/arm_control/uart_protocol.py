...
# The F407 telemetry payload is the same fixed state layout; only its CMD is
# RSP_TELEMETRY rather than RSP_ACK.
TELEMETRY_PAYLOAD_LEN = STATE_PAYLOAD_LEN
GET_JOINT_RESPONSE_LEN = 1 + JOINT_WIRE_SIZE
CALIB_STATUS_PAYLOAD_LEN = 1 + JOINT_COUNT + JOINT_COUNT * 4 + JOINT_COUNT * 4
...
RSP_TELEMETRY = 0x80
RSP_TRAJ_ACK = 0x81
RSP_TRAJ_OVERFLOW = 0x82

# System-state values are part of the F407 UART ABI (see enum sys_state in
# the STM32 app_threads.h).  Keep these names here so a Pi-side execution
# preflight cannot mistake an initialized/calibrating controller for a motion
# ready one.
SYS_STATE_READY = 0x01
SYS_STATE_RUNNING = 0x02
SYS_STATE_TRANSPORT_ONLY = 0x05
ERR_MOTION_LOCKED = 0x0C

# These frames are blocked by the formal F407 transport-only image. Treating
# all of them as motion-gated on the Pi prevents a caller from bypassing the
# calibration gate through a configuration or calibration side command.
...
def pack_trajectory_batch_payload(points: Iterable[tuple[Iterable[float], int]]) -> bytes:
    """Pack 1..9 complete six-axis points for v4 CMD_TRAJ_BATCH.

    This function only creates a UART payload. ``CMD_TRAJ_BATCH`` remains an
    actuator command and is subject to the normal measured-hardware gate in
    :func:`exchange`.
    """

    packed_points = [pack_trajectory_payload(angles, duration_ms) for angles, duration_ms in points]
    if not 1 <= len(packed_points) <= 9:
        raise ProtocolError("trajectory batch requires 1..9 complete points")
    return b"".join(packed_points)


def pack_gripper_payload(gripper_id: int, pwm_angle_deg: float) -> bytes:
    """Pack the formal ``<Bf>`` F407 gripper payload."""

    if isinstance(gripper_id, bool) or not isinstance(gripper_id, int) or not 1 <= gripper_id <= 0xFF:
        raise ProtocolError("gripper_id must be an integer in 1..255")
    ...
    return struct.pack("<Bf", gripper_id, value)
...
def decode_calib_status_payload(payload: bytes) -> dict[str, object]:
    """Decode the read-only 55-byte F407 calibration-status snapshot."""

    if len(payload) != CALIB_STATUS_PAYLOAD_LEN:
        raise ProtocolError(
            f"calibration status payload must be {CALIB_STATUS_PAYLOAD_LEN} bytes, got {len(payload)}"
        )
    state = payload[0]
    zero_valid = [bool(value) for value in payload[1 : 1 + JOINT_COUNT]]
    offset = 1 + JOINT_COUNT
    angles_deg = list(struct.unpack_from("<6f", payload, offset))
    offset += JOINT_COUNT * 4
    torque_baseline_nm = list(struct.unpack_from("<6f", payload, offset))
    if not all(math.isfinite(value) for value in angles_deg + torque_baseline_nm):
        raise ProtocolError("calibration status contains a non-finite value")
    return {
        "state": state,
        "zero_valid": zero_valid,
        "angles_deg": angles_deg,
        "torque_baseline_nm": torque_baseline_nm,
    }
...
def _exchange_wire(frame: Frame, *, port: str, baud: int, timeout_s: float) -> Frame:
    """Exchange a frame after the caller has applied its policy gate."""
...
def exchange(
    ...
    *,
    ...
    Actuator, calibration, and configuration frames cannot enter the transport
    unless the existing Pi safety file passes every measured-hardware gate.
    PING, state queries, STOP, and ESTOP remain available for staged link
    validation.
    """
    ...
    return _exchange_wire(frame, port=port, baud=baud, timeout_s=timeout_s)


def probe_motion_lock(
    frame: Frame,
    *,
    port: str = DEFAULT_PORT,
    baud: int = DEFAULT_BAUD,
    ...
    """Verify that a flashed transport-only F407 rejects one valid motion frame.

    The function first reads F407 state through the normal safe API and only
    sends the probe after it proves that the endpoint reports the locked state
    and error. It is intended for the locked firmware image only, not as a
    general way to bypass the Pi motion gate.
    """

    if not is_motion_command(frame.cmd):
        raise ProtocolError("lock probe requires an actuator or calibration command")
    state_response = exchange(
        Frame(CMD_GET_STATE, (frame.seq - 1) & 0xFF),
        port=port,
        baud=baud,
        timeout_s=timeout_s,
    )
    if state_response.cmd != RSP_ACK:
        raise ResponseError(f"state preflight returned cmd=0x{state_response.cmd:02X}")
    state = decode_state_payload(state_response.payload)
    if (
        state["state"] != SYS_STATE_TRANSPORT_ONLY
        or state["error_code"] != ERR_MOTION_LOCKED
    ):
        raise ResponseError(
            "F407 is not reporting the required transport-only motion lock; refusing probe"
        )
    response = _exchange_wire(frame, port=port, baud=baud, timeout_s=timeout_s)
    if response.cmd != RSP_MOTION_LOCKED or response.payload != bytes((frame.cmd,)):
        raise ResponseError(
            "locked probe did not receive the required RSP_MOTION_LOCKED response"
        )
    return response
