...
# The GIF face supports these named states too.  Keep the Canvas preview at
# feature parity so a user never sees a silent fallback to idle just because a
# different XiaoU screen is active.
STYLE_MAP.update(
    {
        "question": FaceStyle(0.88, 0.98, 0.16, 0.0, 0.22, -0.14, 0.03, 0.12, 0.16, 0.0, 0.0, "", "question"),
        "mock": FaceStyle(0.74, 0.72, 0.28, 0.0, 0.10, -0.18, 0.48, 0.10, 0.20, 0.0, 0.0, "", "mock"),
        "smile": FaceStyle(0.42, 0.35, 0.0, 0.0, -0.04, -0.04, 0.88, 0.10, 0.45, 0.0, 0.0, "heart", "smile"),
        "laugh": FaceStyle(0.26, 0.22, 0.0, 0.0, -0.08, -0.08, 0.98, 0.20, 0.62, 0.0, 0.0, "heart", "laugh"),
        "celebrate": FaceStyle(0.38, 0.36, 0.0, 0.0, -0.06, -0.06, 0.92, 0.18, 0.56, 0.0, 0.0, "heart", "celebrate"),
        "shocked": FaceStyle(0.96, 1.0, 0.0, 0.0, 0.16, 0.16, 0.0, 0.46, 0.12, 0.18, 0.0, "", "shocked"),
    }
)
...
class CanvasFaceAnimator:
    ...
    def resume_live_state(self) -> None:
        """Resume reading the existing shared face state after a local preview.

        This intentionally does not write ``face_state.json``.  It lets the
        Control Tower preview a task mood without interrupting voice/dialog
        expressions shown on the physical face screen.
        """
