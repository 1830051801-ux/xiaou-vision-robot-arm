...
TARGET_ALIASES = {
    "cola": {"cola", "coke"},
    "earphone": {"earphone", "headphone", "headphones"},
    "Coffee cup": {"Coffee cup", "Mug", "Measuring cup", "Bottle", "Wine glass", "cup"},
    "Bottle": {"Bottle", "Coffee cup", "Wine glass", "Mug", "bottle"},
    "bottle": {"Bottle", "bottle"},
    "Pen": {"Pen", "Pencil", "Scissors", "pen"},
    "Mobile phone": {"Mobile phone", "Computer mouse"},
    "Book": {"Book", "Computer keyboard"},
    "Computer keyboard": {"Computer keyboard", "Book"},
    "Computer mouse": {"Computer mouse", "Mobile phone"},
    "Scissors": {"Scissors", "Pen", "Pencil"},
}

SMALL_OBJECT_TARGETS = {"Pen", "pen", "Pencil", "pencil"}
PEN_TARGETS = {"Pen", "pen", "Pencil", "pencil"}

# Pen-specific: minimum aspect ratio to accept as a real pen
PEN_MIN_ASPECT = 1.8          # YOLO boxes include background; keep this moderate and rely on stability/limits too
PEN_MIN_CONF = 0.57           # Small tightening to reduce false pen picks without making detection too strict
...
def _pen_line_fallback_detection(frame: np.ndarray) -> Detection | None:
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, threshold=18, minLineLength=22, maxLineGap=6)
    if lines is None:
        return None
...
def find_stable_target(
    ...
    target_area_ratio = min_area_ratio * 0.45 if target in SMALL_OBJECT_TARGETS else min_area_ratio
    target_min_stable = max(2, min_stable - 1) if target in SMALL_OBJECT_TARGETS else min_stable
    try:
