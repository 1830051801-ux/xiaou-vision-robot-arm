...
- 把离线仿真、协议回放或数值收敛写成真实机械臂验收结果；现场功能条目应注明实际运行来源，并把仿真回归与现场数据分开统计；
...
真实上机应另建现场记录，包含设备身份、固件哈希、标定版本、急停检查、反馈新鲜度和运行结果。GitHub 提交本身不等于硬件验收。

## 2026-09-23 数据归档

最新现场工作簿见 [`evidence/field_validation_20260923/`](evidence/field_validation_20260923/)。目录保留原始文件 SHA-256、22 个工作表的逐行导出和派生指标。工作簿内不同样本范围（例如 VLA 20 条逐条表与 30 条汇总表）按来源分别呈现，不在发布文档中强行合并；目标训练机规格与工作簿记录的实际训练设备也分开记录。
