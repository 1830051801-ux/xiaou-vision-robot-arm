...
反馈新鲜度修正。树莓派不编译 Keil，也不保存或下发 F407 二进制；Pi 保存四个修改后
的 C/H 源码镜像，且只运行 UART 验收脚本。
...
已修改的四个文件会保存在 Pi 的以下目录，保留 `BasicSetting_DaRanRobot` 内的相对路径：

```text
/home/pi/raspi_robot_ai_transport_only_20260808/firmware_staging/F407_J5_feedback_freshness_20260808/BasicSetting_DaRanRobot/
```

这是供 Keil 机器取回的源码镜像，不是 Pi Python 运行时模块，不会被 `joint_status_probe.py`
加载或执行。没有上传 `.hex`、`.bin`、`.axf` 或下载脚本。
